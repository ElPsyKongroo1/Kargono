...
{
	constexpr uint32_t k_MaxFramebufferSize{ 8192 };
	...
	void Framebuffer::Invalidate()
	{
		...
		if (m_ColorAttachmentSpecifications.size())
		{
			...
			for (size_t i = 0; i < m_ColorAttachmentIDs.size(); i++)
			{
				...
				switch (m_ColorAttachmentSpecifications[i].m_DataFormat)
				{
				case Kargono::Rendering::FramebufferDataFormat::RGBA8:
					Utility::AttachColorTexture(m_ColorAttachmentIDs[i], m_Specification.m_Samples, GL_RGBA8, GL_RGBA,
				...
				case Kargono::Rendering::FramebufferDataFormat::RED_INTEGER:
					Utility::AttachColorTexture(m_ColorAttachmentIDs[i], m_Specification.m_Samples, GL_R32I, GL_RED_INTEGER,
						m_Specification.m_Width, m_Specification.m_Height, static_cast<int>(i));
					break;
				}
			}
		...
		if (m_DepthAttachmentSpecification.m_DataFormat != Kargono::Rendering::FramebufferDataFormat::None)
		{
			Utility::CreateTextures(multisample, &m_DepthAttachmentID, 1);
			Utility::BindTexture(multisample, m_DepthAttachmentID);
			switch (m_DepthAttachmentSpecification.m_DataFormat)
			{
			case Kargono::Rendering::FramebufferDataFormat::DEPTH24STENCIL8:
				Utility::AttachDepthTexture(m_DepthAttachmentID, m_Specification.m_Samples,
	...
	void Framebuffer::RegisterBuffer(const FramebufferSpecification& spec)
	{
		m_Specification = spec;

		// For Loop organizes specifications into color or depth attachments that are held
		//		locally
		for (Kargono::Rendering::FramebufferDataSpecification attachement : m_Specification.m_Attachments.m_Attachments)
		{
			if (!Utility::IsDepthFormat(attachement.m_DataFormat))
			{
	...
	void Framebuffer::DeregisterBuffer()
	{
		glDeleteFramebuffers(1, &m_RendererID);
		glDeleteTextures(static_cast<GLsizei>(m_ColorAttachmentIDs.size()), m_ColorAttachmentIDs.data());
		glDeleteTextures(1, &m_DepthAttachmentID);
	...
	void Framebuffer::DisplayToDefaultFrameBuffer()
	{
		// Bind default framebuffer
		Unbind();

		// Bind the screen space shader
		glUseProgram(FrameBufferService::GetScreenSpaceShaderProgram());

		// Draw the quad onto the default frame buffer
		glBindVertexArray(FrameBufferService::GetScreenSpaceQuadVAO());
	...
	{
		if (width == 0 || height == 0 || width > k_MaxFramebufferSize || height > k_MaxFramebufferSize)
		{
	...
	int32_t Framebuffer::ReadPixel(uint32_t attachmentIndex, int32_t x, int32_t y)
	{
		KG_ASSERT(attachmentIndex < m_ColorAttachmentIDs.size(), "Color attachment selection is out of bounds!");
		KG_ASSERT(m_ColorAttachmentSpecifications.at(attachmentIndex).m_DataFormat == Kargono::Rendering::FramebufferDataFormat::RED_INTEGER, "Attempt to use ReadPixel on a buffer that is not RED_INTEGER");
		glReadBuffer(GL_COLOR_ATTACHMENT0 + attachmentIndex);
		int pixelData;
		glReadPixels(x, y, 1, 1, GL_RED_INTEGER, GL_INT, &pixelData);
		return pixelData;
	}
	void Framebuffer::SetAttachment(uint32_t attachmentIndex, int value)
	{
		KG_ASSERT(attachmentIndex < m_ColorAttachmentIDs.size(), "Color attachment selection is out of bounds!")
		...
		glClearTexImage(m_ColorAttachmentIDs[attachmentIndex], 0,
			Utility::KargonoTextureFormatToGL(spec.m_DataFormat), GL_INT, &value);
	}
	...
	unsigned int FrameBufferService::GetScreenSpaceQuadVAO()
	{
		return s_ScreenSpaceQuadVAO;
	}
	unsigned int FrameBufferService::GetScreenSpaceQuadVBO()
	{
		return s_ScreenSpaceQuadVBO;
	}
	unsigned int FrameBufferService::GetScreenSpaceShaderProgram()
	{
