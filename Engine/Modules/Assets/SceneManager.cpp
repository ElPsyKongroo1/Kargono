	...
	Ref<Scenes::Scene> SceneManager::DeserializeAsset(Assets::AssetInfo& assetInfo, const std::filesystem::path& assetPath)
	{
		...
		if (entities)
		{
			for (const YAML::Node& entity : entities)
			{
				...
				ECS::Entity deserializedEntity = 
					newScene->m_EntityRegistry.CreateEntityWithUUID(uuid, name);
				...
				if (aiStateNode)
				{
					...
					if (component.m_CurrentStateHandle != Assets::k_EmptyHandle && !component.m_CurrentStateReference)
					{
					...
					if (component.m_PreviousStateHandle != Assets::k_EmptyHandle && !component.m_PreviousStateReference)
					{
					...
					if (component.m_GlobalStateHandle != Assets::k_EmptyHandle && !component.m_GlobalStateReference)
					{
		...
		{
			ECS::Entity primaryCameraEntity = newScene->m_EntityRegistry.GetEntityByUUID(primaryCameraID);
