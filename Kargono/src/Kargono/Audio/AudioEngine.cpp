...
OpenAL_ErrorCheck(FUNCTION_CALL)
	...
	AudioBuffer::AudioBuffer(const std::string& fileLocation)
	{
		// Create Buffers
		uint32_t channels = 0;
		uint32_t sampleRate = 0;
		uint64_t totalPcmFrameCount = 0;
		ScopedBuffer pcmData{};
		drwav_int16* pSampleData = drwav_open_file_and_read_pcm_frames_s16(fileLocation.c_str(), &channels, &sampleRate, &totalPcmFrameCount, nullptr);
		if (!pSampleData)
		{
		...
		if ((totalPcmFrameCount * channels) > drwav_uint64(std::numeric_limits<size_t>::max()))
		{
			KG_CORE_ERROR("Too much data in file for 32bit addressed vector");
			drwav_free(pSampleData, nullptr);
			return;
		}
		pcmData.Allocate(size_t(totalPcmFrameCount * channels) * 2);
		std::memcpy(pcmData.Data(), pSampleData, pcmData.Size() /*two bytes in s16*/);
		drwav_free(pSampleData, nullptr);
		// Create OpenAL buffer object and fill its data buffer with audio
		alec(alGenBuffers(1, &m_BufferID));
		alec(alBufferData(m_BufferID, channels > 1 ? AL_FORMAT_STEREO16 : AL_FORMAT_MONO16, pcmData.Data(), static_cast<ALsizei>(pcmData.Size()) /*Two bytes per sample*/, sampleRate));
	...
	void AudioEngine::Terminate()
	{
		...
		AssetManager::ClearAudioRegistry();
