...
namespace Kargono
{
	...
	namespace Utils {
		...
		static GLenum KargonoTextureFormatToGL(FramebufferTextureFormat format)
		{
			switch (format)
			{
	...
	void OpenGLFramebuffer::Invalidate()
	{
		...
		{
			KG_CORE_ASSERT(m_ColorAttachments.size() <= 4, "Core error, we only support a maximum of 4 color attachments");
			GLenum buffers[4] = {
	...
	{
		KG_CORE_ASSERT(attachmentIndex < m_ColorAttachments.size(), "Color attachment selection is out of bounds!")
		glReadBuffer(GL_COLOR_ATTACHMENT0 + attachmentIndex);
		int pixelData;
		glReadPixels(
			x, y, 1, 1, GL_RED_INTEGER, GL_INT, &pixelData);
		return pixelData;
	}
	void OpenGLFramebuffer::ClearAttachment(uint32_t attachmentIndex, int value)
	{
		...
		glClearTexImage(m_ColorAttachments[attachmentIndex], 0,
			Utils::KargonoTextureFormatToGL(spec.TextureFormat), GL_INT, &value);
	}
}
