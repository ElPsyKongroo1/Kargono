...
#include "Windows/ParticleEmitter/ParticleEmitterWindow.h"
...
{
	void ParticleEmitterWindow::OpenCreateDialog(std::filesystem::path& createLocation)
	{
	...
	void ParticleEmitterWindow::ResetWindowResources()
	{
		// Reset editor user interface
		m_EditorEmitterHandle = 0;
		m_EditorEmitter = nullptr;

		// Reset properties panel data
		m_PropertiesPanel->ClearPanelData();
	}

	void ParticleEmitterWindow::OnRefreshData()
	{
	}
	...
	void ParticleEmitterWindow::InitPanels()
	{
		// Initialize panels
		m_ViewportPanel = CreateScope<Panels::ParticleEmitterViewportPanel>();
		m_PropertiesPanel = CreateScope<Panels::ParticleEmitterPropertiesPanel>();
	}

	bool ParticleEmitterWindow::OnInputEvent(Events::Event* event)
	{
		if (m_EditorEmitter)
		{
	...
	void ParticleEmitterWindow::OnUpdate(Timestep ts)
	{
	...
	void ParticleEmitterWindow::OnEditorUIRender()
	{
		KG_PROFILE_FUNCTION();
	...
	bool ParticleEmitterWindow::OnKeyPressedEditor(Events::KeyPressedEvent event)
	{
		bool handled{ false };
	...
	bool ParticleEmitterWindow::OnAssetEvent(Events::Event* event)
	{
			...
			manageAsset->GetAction() == Events::ManageAssetAction::Delete)
		{
			...
			if (m_EditorEmitter)
			{
				Assets::AssetService::RemoveScriptFromUserInterface(m_EditorEmitter, manageAsset->GetAssetID());
			}
		}
			...
			manageAsset->GetAction() == Events::ManageAssetAction::Delete)
		{
			...
			if (manageAsset->GetAssetID() != m_EditorEmitterHandle)
			{
				return false;
			}
	...
	void ParticleEmitterWindow::OpenAssetInEditor(std::filesystem::path& assetLocation)
	{
		...
		if (m_EditorEmitterHandle == assetHandle)
		{
			// Open the user interface window in the editor
			EngineService::SubmitToMainThread([]()
			{
				s_EditorApp->SetActiveEditorWindow(ActiveEditorUIWindow::ParticleEmitterWindow);
			});
			return;
		}
