...
namespace Kargono::Panels
{
	...
	{
	    Assets::AssetRef<Scripting::Script> script = Assets::s_ScriptManager.GetAssetByHandle(m_ActiveScriptHandle);
		if (!script)
		{
		...
		bool successful = Assets::s_ScriptManager.UpdateAsset(spec);
		if (!successful)
		{
	...
	void ScriptEditorPanel::InitializeEditScriptPopup()
	{
		...
		{
			m_EditWidgets.m_EditName.m_CurrentOption = Assets::s_ScriptManager.GetAssetByHandle(m_ActiveScriptHandle)->m_ScriptName;
			m_EditWidgets.m_SelectSectionLabel.m_CurrentOption.m_Label = Assets::s_ScriptManager.GetAssetByHandle(m_ActiveScriptHandle)->m_SectionLabel;
		};
