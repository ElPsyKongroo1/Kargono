...
namespace Kargono::Panels
{
	...
	void CustomComponentPanel::InitializeOpeningPanel()
	{
		...
		m_OpenComponentPopup.m_ConfirmAction = [&](const EditorUI::OptionEntry& selection)
		{
			...
			if (!Assets::s_CustomComponentManager.GetAssetRegistry().contains(selection.m_Handle))
			{
		...
		m_CreateComponentPopup.m_ConfirmAction = [&]()
		{
			...
			for (auto& [id, asset] : Assets::s_CustomComponentManager.GetAssetRegistry())
			{
			...
			m_EditorCustomComponent = CreateRef<ECSInternal::CustomComponent>(*Assets::s_CustomComponentManager.GetAssetByHandle(m_EditorCustomComponentHandle));
			m_MainHeader.m_EditColorActive = false;
			m_MainHeader.m_Label = Assets::s_CustomComponentManager.GetAssetRegistry().at(
	...
	void CustomComponentPanel::InitializeComponentFieldsSection()
	{
		...
		{
			Assets::s_CustomComponentManager.UpdateAsset(m_EditorCustomComponent);
			Scripting::ScriptCompilerService::GetActiveContext().m_ActiveLanguageDefinition.CreateLanguageDef();
			m_MainHeader.m_EditColorActive = false;
		});
	...
	void CustomComponentPanel::OpenAssetInEditor(std::filesystem::path& assetLocation)
	{
		...
		Assets::AssetHandle assetHandle = Assets::s_CustomComponentHandleFromFileLocationManager.GetAssetByHandle(relativePath);
	...
	{
		m_EditorCustomComponent = CreateRef<ECSInternal::CustomComponent>(*Assets::s_CustomComponentManager.GetAssetByHandle(newHandle));
