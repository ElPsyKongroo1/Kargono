...
namespace Kargono::Panels
{
	...
	void InputMapPanel::OnOpenInputMap(Assets::AssetHandle newHandle)
	{
		...
		m_EditorInputMap = Assets::s_InputMapManager.GetAssetByHandle(newHandle);
		m_EditorInputMapHandle = newHandle;
		m_MainHeader.m_EditColorActive = false;
		m_MainHeader.m_Label = Assets::s_InputMapManager.GetAssetRegistry().at(
	...
	void InputMapPanel::OpenAssetInEditor(std::filesystem::path& assetLocation)
	{
		...
		Assets::AssetHandle assetHandle = Assets::s_InputMapHandleFromFileLocationManager.GetAssetByHandle(relativePath);
	...
	void InputMapPanel::InitializeOpeningScreen()
	{
		...
		m_OpenInputMapPopupSpec.m_PopupAction = [&](EditorUI::SelectOptionWidget& spec)
		{
			...
			for (auto& [handle, asset] : Assets::s_InputMapManager.GetAssetRegistry())
			{
		...
		m_OpenInputMapPopupSpec.m_ConfirmAction = [&](const EditorUI::OptionEntry& selection)
		{
			...
			if (!Assets::s_InputMapManager.GetAssetRegistry().contains(selection.m_Handle))
			{
		...
		m_CreateInputMapPopupSpec.m_ConfirmAction = [&]()
		{
			...
			m_EditorInputMap = Assets::s_InputMapManager.GetAssetByHandle(m_EditorInputMapHandle);
			m_MainHeader.m_EditColorActive = false;
			m_MainHeader.m_Label = Assets::s_InputMapManager.GetAssetRegistry().at(
				m_EditorInputMapHandle).Data.FileLocation.filename().string();
			OnRefreshData();
	...
	void InputMapPanel::InitializeInputMapHeader()
	{
		...
		{
			Assets::s_InputMapManager.UpdateAsset(m_EditorInputMap);
			m_MainHeader.m_EditColorActive = false;
		});
	...
	void InputMapPanel::InitializeKeyboardScreen()
	{
		...
		m_KeyboardOnUpdateAddPopup.m_ConfirmAction = [&]()
		{
			...
			{
				script = Assets::s_ScriptManager.GetAssetByHandle(m_KeyboardOnUpdateAddFunction.m_CurrentOption.m_Handle);
			}
		...
		m_KeyboardOnUpdateEditPopup.m_ConfirmAction = [&]()
		{
			...
			{
				script = Assets::s_ScriptManager.GetAssetByHandle(m_KeyboardOnUpdateEditFunction.m_CurrentOption.m_Handle);
			}
		...
		m_KeyboardOnKeyPressedAddPopup.m_ConfirmAction = [&]()
		{
			...
			{
				script = Assets::s_ScriptManager.GetAssetByHandle(m_KeyboardOnKeyPressedAddFunction.m_CurrentOption.m_Handle);
			}
		...
		m_KeyboardOnKeyPressedEditPopup.m_ConfirmAction = [&]()
		{
			...
			{
				script = Assets::s_ScriptManager.GetAssetByHandle(m_KeyboardOnKeyPressedEditFunction.m_CurrentOption.m_Handle);
			}
