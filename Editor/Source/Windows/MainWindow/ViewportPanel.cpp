...
namespace Kargono::Panels
{
	...
	void ViewportPanel::OnUpdate(Timestep ts)
	{
		...
		if (s_MainWindow->m_SceneState == SceneState::Play)
		{
			...
			RuntimeUI::RuntimeUIService::GetActiveContext().GetActiveUI()->OnRenderViewport(m_ViewportData);
	...
	void ViewportPanel::OnEditorUIRender()
	{
		...
		{
			if (isHovered && ImGui::IsItemClicked(ImGuiMouseButton_Left) && ImGui::GetIO().WantCaptureMouse)
			{
		...
		else
		{
			if (isHovered && ImGui::IsItemClicked(ImGuiMouseButton_Left) && ImGui::GetIO().WantCaptureMouse)
			{
				...
				if (m_ViewportHovered && !ImGuizmo::IsOver() && !Input::InputService::IsKeyPressed(Key::LeftAlt))
				{
					Events::MouseButtonPressedEvent event{ Mouse::ButtonLeft };
	...
	void ViewportPanel::OnMouseButtonPressedEvent(const Events::MouseButtonPressedEvent& event)
	{
		...
		{
			// Handle on press
			uiContext.GetActiveUI()->OnPressByIndex({ uiContext.GetActiveUIHandle(),
			...
			uiContext.m_ActiveUI->m_InteractState.SetEditingWidgetByHandle({ uiContext.GetActiveUIHandle(),
				m_HoveredWindowWidgetID });

			// Throw a mouse pressed event
			Events::MouseButtonPressedEvent mousePressedEvent{ Mouse::ButtonLeft };
			uiContext.GetActiveUI()->OnMouseButtonPressedEvent(mousePressedEvent);
	...
	void ViewportPanel::HandleUIMouseHovering()
	{
		...
		{
			uiContext.m_ActiveUI->m_InteractState.ClearHoveredWidget();
			return;
		}
	...
	void ViewportPanel::OnOverlayRender()
	{
		...
		if (s_MainWindow->m_ShowPhysicsColliders)
		{
			...
			// Circle Colliders
			{
				auto view = activeScene->m_EntityRegistry.GetView<Transform, Physics2D::CircleCollider2D>();
				for (ECSInternal::EntityID entity : view)
				{
					...
					Physics2D::CircleCollider2D& cc2d = activeScene->m_EntityRegistry.m_Registry.GetComponent<Physics2D::CircleCollider2D>(entity).value();
			...
			// Box Colliders
			{
				auto view = activeScene->m_EntityRegistry.GetView<Transform, Physics2D::BoxCollider2D>();
				for (ECSInternal::EntityID entity : view)
				{
					Transform& tc = activeScene->m_EntityRegistry.m_Registry.GetComponent<Transform>(entity).value();
					Physics2D::BoxCollider2D& bc2d = activeScene->m_EntityRegistry.m_Registry.GetComponent<Physics2D::BoxCollider2D>(entity).value();
	...
	void ViewportPanel::DrawSplines()
	{
		...
		{
			Rendering::Shader::SetDataAtInputLocation<Math::vec4>(Utility::ImVec4ToMathVec4(EditorUI::k_PureWhite),
				Utility::FileSystem::CRCFromString("a_Color"),
				s_LineInputSpec.m_Buffer, s_LineInputSpec.m_Shader);
