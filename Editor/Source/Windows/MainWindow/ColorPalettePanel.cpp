...
namespace Kargono::Panels
{
	void ColorPalettePanel::InitializeOpeningScreen()
	{
		...
		m_OpenColorPalettePopup.m_PopupAction = [&](EditorUI::SelectOptionWidget& spec)
		{
			...
			for (auto& [handle, asset] : Assets::s_ColorPaletteManager.GetAssetRegistry())
			{
		...
		m_OpenColorPalettePopup.m_ConfirmAction = [&](const EditorUI::OptionEntry& selection)
		{
			...
			if (!Assets::s_ColorPaletteManager.GetAssetRegistry().contains(selection.m_Handle))
			{
		...
		m_CreateColorPalettePopup.m_ConfirmAction = [&]()
		{
			...
			for (auto& [id, asset] : Assets::s_ColorPaletteManager.GetAssetRegistry())
			{
			...
			m_EditorColorPalette = Assets::s_ColorPaletteManager.GetAssetByHandle(m_EditorColorPaletteHandle);
			m_MainHeader.m_EditColorActive = false;
			m_MainHeader.m_Label = Assets::s_ColorPaletteManager.GetAssetRegistry().at(
	...
	void ColorPalettePanel::InitializeDisplayColorPaletteScreen()
	{
		...
		m_MainHeader.AddToSelectionList("Save", [&]()
			{
				Assets::s_ColorPaletteManager.UpdateAsset(m_EditorColorPalette);
				m_MainHeader.m_EditColorActive = false;
			});
	...
	void ColorPalettePanel::OpenAssetInEditor(std::filesystem::path& assetLocation)
	{
		...
		Assets::AssetHandle assetHandle = Assets::s_ColorPaletteHandleFromFileLocationManager.GetAssetByHandle(relativePath);
	...
	{
		m_EditorColorPalette = Assets::s_ColorPaletteManager.GetAssetByHandle(newHandle);
