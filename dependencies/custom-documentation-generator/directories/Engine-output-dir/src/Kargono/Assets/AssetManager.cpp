...
OpenAL_ErrorCheck(FUNCTION_CALL)
	...
	std::tuple<AssetHandle, Ref<Kargono::Shader>> AssetManager::GetShader(const Shader::ShaderSpecification& shaderSpec)
	{
		KG_CORE_ASSERT(Projects::Project::GetActive(), "There is no active project when retrieving shader!");
	...
	void AssetManager::CreateShaderIntermediate(const Shader::ShaderSource& shaderSource, Assets::Asset& newAsset, const Shader::ShaderSpecification& shaderSpec,
	...
	std::unordered_map<AssetHandle, Ref<Kargono::Font>> AssetManager::s_Fonts {};
	...
	Ref<Kargono::Font> AssetManager::InstantiateFontIntoMemory(Assets::Asset& asset)
	{
		Assets::FontMetaData metadata = *static_cast<Assets::FontMetaData*>(asset.Data.SpecificFileData.get());
		Buffer currentResource{};
		currentResource = FileSystem::ReadFileBinary(Projects::Project::GetAssetDirectory() / asset.Data.IntermediateLocation);
		Ref<Kargono::Font> newFont = CreateRef<Kargono::Font>();
	...
	Ref<Kargono::Font> AssetManager::GetFont(const AssetHandle& handle)
	{
		...
		if (s_FontRegistry.contains(handle))
		{
			...
			Ref<Kargono::Font> newFont = InstantiateFontIntoMemory(asset);
		...
		if (entity.HasComponent<ScriptComponent>())
		{
			...
			if (fields.size() > 0)
			{
				...
				for (const auto& [name, field] : fields)
				{
					...
					out << YAML::Key << "Type" << YAML::Value << Kargono::Utils::ScriptFieldTypeToString(field.Type);
					...
					switch (field.Type)
					{
						...
						WRITE_SCRIPT_FIELD(ScriptFieldType::Vector2, Math::vec2);
						WRITE_SCRIPT_FIELD(ScriptFieldType::Vector3, Math::vec3);
						WRITE_SCRIPT_FIELD(ScriptFieldType::Vector4, Math::vec4);
					...
					if (scriptFields)
					{
						...
						if (entityClass)
						{
							...
							for (auto scriptField : scriptFields)
							{
								...
								ScriptFieldType type = Kargono::Utils::ScriptFieldTypeFromString(typeString);
								...
								switch (type)
								{
									...
									READ_SCRIPT_FIELD(ScriptFieldType::Vector2, Math::vec2);
									READ_SCRIPT_FIELD(ScriptFieldType::Vector3, Math::vec3);
									READ_SCRIPT_FIELD(ScriptFieldType::Vector4, Math::vec4);
				...
				if (shapeComponent)
				{
					...
					if (shapeComponent["ShaderHandle"])
					{
						...
						if (sc.CurrentShape != Shape::ShapeTypes::None)
						{
							...
							{
								sc.Vertices = CreateRef<std::vector<Math::vec3>>(Shape::ShapeTypeToShape(sc.CurrentShape).GetIndexVertices());
								sc.Indices = CreateRef<std::vector<uint32_t>>(Shape::ShapeTypeToShape(sc.CurrentShape).GetIndices());
								sc.TextureCoordinates = CreateRef<std::vector<Math::vec2>>(Shape::ShapeTypeToShape(sc.CurrentShape).GetIndexTextureCoordinates());
							}

							if (sc.ShaderSpecification.RenderType == Shape::RenderingType::DrawTriangle)
							{
								sc.Vertices = CreateRef<std::vector<Math::vec3>>(Shape::ShapeTypeToShape(sc.CurrentShape).GetTriangleVertices());
								sc.TextureCoordinates = CreateRef<std::vector<Math::vec2>>(Shape::ShapeTypeToShape(sc.CurrentShape).GetTriangleTextureCoordinates());
							}
						}
					}
				}
	...
	std::tuple<AssetHandle, Ref<Kargono::Scene>> AssetManager::GetScene(const std::filesystem::path& filepath)
	{
		...
		std::filesystem::path scenePath = FileSystem::GetRelativePath(Projects::Project::GetAssetDirectory(), filepath);
	...
	Ref<UIEngine::UIObject> AssetManager::InstantiateUIObject(const Assets::Asset& uiObjectAsset)
	{
