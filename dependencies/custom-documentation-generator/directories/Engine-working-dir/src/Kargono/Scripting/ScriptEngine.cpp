...
namespace Kargono
{
	...
	void ScriptEngine::ReloadAssembly()
	{
		...
		const auto scriptModulePath = Projects::Project::GetAssetDirectory() / Projects::Project::GetActive()->GetConfig().ScriptModulePath;
	...
	void ScriptEngine::InitialAssemblyLoad()
	{
		...
		auto scriptModulePath = Projects::Project::GetAssetDirectory() / Projects::Project::GetActive()->GetConfig().ScriptModulePath;
		status = LoadAppAssembly(scriptModulePath);
		if (!status)
		{
			KG_CORE_ERROR("[ScriptEngine] Could not load app assembly.");
