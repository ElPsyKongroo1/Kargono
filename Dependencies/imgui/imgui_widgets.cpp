...
void ImGui::SeparatorEx(ImGuiSeparatorFlags flags, ImVec4 color, float thickness)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    if (flags & ImGuiSeparatorFlags_Vertical)
    {
        ...
        // Draw
        window->DrawList->AddLine(ImVec2(bb.Min.x, bb.Min.y), ImVec2(bb.Min.x, bb.Max.y), ImGui::ColorConvertFloat4ToU32(color), thickness);
        if (g.LogEnabled)
            LogText(" |");
    }
    else if (flags & ImGuiSeparatorFlags_Horizontal)
    {
        ...
        {
            // Draw
            window->DrawList->AddLine(bb.Min, ImVec2(bb.Max.x, bb.Min.y), ImGui::ColorConvertFloat4ToU32(color), thickness);
            if (g.LogEnabled)
...
void ImGui::Separator(float thickness, ImVec4 color)
{
    ImGuiContext& g = *GImGui;
    ImGuiWindow* window = g.CurrentWindow;
    if (window->SkipItems)
    ...
    SeparatorEx(flags, color, thickness);
}
...
bool ImGui::DragScalar(const char* label, ImGuiDataType data_type, void* p_data, float v_speed, const void* p_min, const void* p_max, const char* format, ImGuiSliderFlags flags)
{
    ...
    RenderTextClipped(frame_bb.Min, frame_bb.Max, value_buf, value_buf_end, NULL,
        flags & ImGuiSliderFlags_CenterText ? ImVec2(0.5f, 0.0f) : ImVec2(0.0f, 0.0f));
...
void ImGui::ColorEditRestoreHS(const float* col, float* H, float* S, float* V)
{
...
void ImGui::RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz, float bar_w, float alpha)
{
